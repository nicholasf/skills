...
**Files changed:** `pyproject.toml`, `tests/__init__.py`, `tests/conftest.py`, `tests/test_read_file.py`, `tests/test_write_file.py`, `tests/test_bash.py`, `tests/test_find_files.py`, `tests/test_grep.py`, `tests/test_edit_file.py`, `tests/test_list_directory.py`, `tests/test_git_diff.py`
**Summary:** Pond wrote conftest, test_read_file, and test_write_file before running out of context. Remaining six test files written by Claude. All tests called tools via `.invoke({...})` — LangChain `StructuredTool` objects are not directly callable. `list_directory` exclusions fixed (single `|` pattern → separate `-not -path` clauses).
